...
function HUDRaidWW2MiniGame:init(params)
	self._parent = parent
	...
	self._timer_text = self._hud_panel:text({
		...
		visible = self._tweak_data.uses_timer == true,
		color = Color(1, 0.6, 0.3),
		valign = "center",
		name = self._child_ivalid_name_text,
		font = "fonts/font_large_mf"
	})
	self._timer_text:set_center_y(self._lockpick_texture:center_y())
...
function HUDRaidWW2MiniGame:set_timer(t)
	self._timer_text:set_text(string.format("%.2f", t))
	play_color(self._timer_text, t == 0 and Color(1, 0.3, 0.3) or Color(1, 0.6, 0.3))
end
...
function HUDRaidWW2MiniGame:show()
    ...
    for i = 1, self._tweak_data.number_of_circles do
		...
		local circle = CircleBitmapGuiObject:new(self._hud_panel, {
			blend_mode = "add",
			layer = 2,
			image = tweak_data.interaction.rww2_circles[math.min(i, 3)],
...
function HUDRaidWW2MiniGame:set_bar_valid(circle_id, valid)
	...
	circle_data.circle:set_image(tweak_data.interaction.rww2_circles[math.min(circle_id, 3)]..(valid and "" or "_fail"))
